...
lock('Open Details rejects stale navigation completions', app,
  /planDetailsOpenEpochRef = useRef\(0\)[\s\S]*?openEpoch = \+\+planDetailsOpenEpochRef\.current[\s\S]*?openEpoch !== planDetailsOpenEpochRef\.current[\s\S]*?handleOpenPlanChat[\s\S]*?planDetailsOpenEpochRef\.current \+= 1[\s\S]*?handleProfileNavigation[\s\S]*?planDetailsOpenEpochRef\.current \+= 1[\s\S]*?handleSignalCenterNavigation[\s\S]*?planDetailsOpenEpochRef\.current \+= 1[\s\S]*?handleDiscoverNavigation[\s\S]*?planDetailsOpenEpochRef\.current \+= 1/,
...
lock('Onboarding submission rejects stale gate lifecycles', onboardingGate,
  /submitEpochRef = useRef\(0\)[\s\S]*?submitEpochRef\.current \+= 1[\s\S]*?submitEpoch = submitEpochRef\.current[\s\S]*?completeMyOnboarding[\s\S]*?submitEpoch !== submitEpochRef\.current[\s\S]*?submitEpoch === submitEpochRef\.current/,
  'A late onboarding completion must not mutate an unmounted onboarding gate.')
...
lock('Moment mutation follow-up reads reject stale card lifecycles', activityView,
  /handleComment[\s\S]*?loadComments\(\)[\s\S]*?cardEpoch !== cardEpochRef\.current[\s\S]*?handleCommentDelete[\s\S]*?loadComments\(\)[\s\S]*?cardEpoch !== cardEpochRef\.current[\s\S]*?handleDelete[\s\S]*?onDeleted\(\)[\s\S]*?cardEpoch !== cardEpochRef\.current/,
  'Moment mutation follow-up work must not continue into a replacement card lifecycle.')
