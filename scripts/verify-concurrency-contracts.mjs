...
requireMatch('Completion DONE releases resume suppression', appShell, /SignalCompletionView[\s\S]*?onDone=\{\(\) => \{[\s\S]*?completionHandoffRef\.current = false[\s\S]*?setCompletionPlanId\(null\)[\s\S]*?setActiveSurface\('activity'\)/i, 'completion suppression must end only when the user exits the historical receipt into Activity')
