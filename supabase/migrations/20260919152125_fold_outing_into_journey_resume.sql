...
-- The canonical journey resume already owns Plan membership and DONE HERE
-- exclusion. Return this member's check-in timestamp from that same snapshot so
-- browser reconciliation does not need a second get_my_active_signal_outing RPC.
...
begin
  ...
  return query
  with journey_candidates(signal_intent_id,signal_group_id,group_state,member_count,activation_threshold,activity_slug,time_window_code,crowd_mode,min_age,max_age,signal_stage,locked_venue,plan_id,plan_details_opened,checked_in_at,priority,sort_at) as (
    select
      ...
      p.id,(pm.live_details_opened_at is not null),
    ...
    select
      ...
      (select svo.payload
       ...
       where svr.signal_group_id=sg.id and svr.state='won' and svr.winner_option_id is not null
       order by svr.round_number desc limit 1),
      null::uuid,false,null::timestamptz,2,sg.updated_at
  ...
  select jc.signal_intent_id,jc.signal_group_id,jc.group_state,jc.member_count,
    ...
    jc.plan_details_opened,jc.checked_in_at
...
is 'Returns the caller live Signal journey, including member check-in authority, while excluding a Plan after that member has marked the outing DONE HERE.';
