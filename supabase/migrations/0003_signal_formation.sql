...
-- ============================================================
-- SIGNAL / joinv2
-- 0003_signal_formation.sql
--
-- Purpose:
--   1. Add required profile gender foundation.
--   2. Require gender + birth date for completed profiles.
--   3. Provide the authoritative Signal formation domain operation.
--
-- IMPORTANT:
--   - birth_date remains authoritative; age is derived.
--   - no fixture UUIDs are embedded here.
--   - grouping policy is resolved by active policy code/version.
--   - this function is backend-only at this stage.
-- ============================================================


-- ============================================================
-- PROFILE GENDER FOUNDATION
-- ============================================================
...
alter table public.user_profiles
  drop constraint user_profiles_completion_check;

alter table public.user_profiles
  add constraint user_profiles_completion_check check (
...
-- ============================================================
-- SIGNAL INTENT HARD IDENTITY
-- ============================================================

drop index if exists public.signal_intents_active_effective_key;

create unique index signal_intents_active_effective_key_v2
...
-- ============================================================
-- AUTHORITATIVE SIGNAL FORMATION
-- ============================================================

-- ============================================================
-- SIGNAL GROUP HARD TIME WINDOW
-- ============================================================
...
-- Historical Signal groups created before this migration did not own
-- their hard matching window. They must remain truthful rather than
-- receiving a fabricated window from later coordination/Plan data.
--
-- New groups, however, must always own the hard matching window.
alter table public.signal_groups
  add constraint signal_groups_time_check
  check (
...
create or replace function public.enforce_signal_group_time_window()
returns trigger
language plpgsql
...
begin
  if new.starts_at is null or new.ends_at is null then
    raise exception 'signal group time window is required';
  end if;

  if new.ends_at <= new.starts_at then
    raise exception 'signal group time window is invalid';
  end if;
...
create trigger signal_groups_require_time_window
before insert or update of starts_at, ends_at
on public.signal_groups
for each row
execute function public.enforce_signal_group_time_window();
...
begin
  ...
  -- Threshold reached.
  --
  -- Move the Signal from FORMING to CONFIRMING.
  -- Membership confirmation itself remains a separate lifecycle
  -- operation; we do NOT fake confirmation here.
  -- ----------------------------------------------------------
...
-- EXECUTION BOUNDARY
--
-- This function accepts an explicit user UUID and therefore must
-- not be directly callable by an untrusted browser client.
-- Frontend/backend authorization is a later integration gate.
-- ============================================================
