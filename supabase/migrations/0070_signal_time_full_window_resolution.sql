...
begin
  ...
  set eligible_participant_count = v_eligible_count,
      activation_threshold = v_policy.activation_threshold,      updated_at = clock_timestamp()
  where id = v_round.id;
  ...
  if v_winner_option_id is not null
     and v_winner_available_count >= v_policy.activation_threshold then
    update public.signal_time_rounds
  ...
  -- Disagreement is not failure while the 10-second window is still open.
