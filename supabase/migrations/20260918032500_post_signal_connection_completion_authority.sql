...
-- Post-Signal connection eligibility is member-scoped: both people must have
-- actually checked in and independently finished their outing. It must not
-- wait for the Plan's scheduled global completion boundary.
...
begin
  ...
  if not exists (
    select 1
    from public.plans p
    join public.plan_memberships pm on pm.plan_id=p.id and pm.user_id=v_user_id
    ...
    raise exception 'completed_signal_outing_required' using errcode='42501';
  end if;

  return query
  select sc.id,pm.user_id,coalesce(up.display_name,'SIGNAL member'),up.avatar_path,
    case when sc.id is null then 'none'
    ...
    case when sc.id is null then 'none'
         ...
         when sc.requested_by=v_user_id then 'outgoing' else 'incoming' end
  from public.plan_memberships pm
  join public.user_profiles up on up.user_id=pm.user_id
  join public.plan_member_outing_completions other_done
    on other_done.plan_id=pm.plan_id and other_done.user_id=pm.user_id
  ...
  where pm.plan_id=p_plan_id
    ...
    and exists (
      select 1 from public.attendance_records ar
      where ar.plan_id=p_plan_id and ar.user_id=pm.user_id
        and ar.evidence_type='self_reported'::public.attendance_evidence_type
    )
    and not public.users_have_block_relation(v_user_id,pm.user_id)
  order by other_done.completed_at,pm.joined_at,pm.id;
end;$function$;
...
as $function$
declare
  v_user_id uuid:=auth.uid(); v_low uuid; v_high uuid;
  v_connection public.signal_connections%rowtype;
  v_now timestamptz:=clock_timestamp(); v_auto_accepted boolean:=false;
begin
  if v_user_id is null then raise exception 'authentication_required' using errcode='42501'; end if;
  ...
  if public.users_have_block_relation(v_user_id,p_target_user_id) then
  ...
  if not exists (
    ...
    join public.plan_member_outing_completions their_done on their_done.plan_id=p.id and their_done.user_id=p_target_user_id
  ...
  ) then raise exception 'shared_completed_signal_outing_required' using errcode='42501'; end if;
  ...
  elsif v_connection.state='pending'::public.signal_connection_state and v_connection.requested_by<>v_user_id then
    update public.signal_connections set state='accepted',responded_at=v_now,updated_at=v_now where id=v_connection.id returning * into v_connection;
  ...
  if v_connection.state='accepted'::public.signal_connection_state then
    ...
    from (values(v_user_id),(p_target_user_id)) as x(u) on conflict(user_id,dedupe_key) do nothing;
  end if;
