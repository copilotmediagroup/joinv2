...
-- Signal formation is invoked by the authenticated Edge Function through the
-- service-role domain client. Keep the block predicate internal and execute it
-- with owner privileges so its reads of private membership/block tables are
-- deterministic and independent of browser RLS/auth.uid() context.
...
is 'Service-role-only internal block compatibility predicate for Signal formation. SECURITY DEFINER permits deterministic reads of private membership/block tables; caller identity remains supplied only by the authenticated signal-form Edge Function.';
