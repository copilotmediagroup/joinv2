...
-- Notification links resolve against current authoritative state,
-- not the historical event that originally created the alert.
...
begin
  ...
  if v_plan_id is null
     and v_notification.related_signal_group_id is not null then
    select p.id into v_plan_id
    from public.plans p
    where p.originating_signal_group_id=
      v_notification.related_signal_group_id
    limit 1;
  end if;
  ...
  if v_notification.related_signal_group_id is not null then
    select
      sgm.originating_signal_intent_id,
      sg.state,
      a.slug
    into
      v_signal_intent_id,
    ...
    where sgm.signal_group_id=v_notification.related_signal_group_id
    ...
    if v_signal_intent_id is not null then
      ...
      where svr.signal_group_id=v_notification.related_signal_group_id
      ...
      return jsonb_build_object(
        'targetType','signal',
        'signalGroupId',v_notification.related_signal_group_id,
...
alter function public.resolve_my_notification_target(uuid)
  owner to postgres;
revoke all on function public.resolve_my_notification_target(uuid)
  from public,anon;
grant execute on function public.resolve_my_notification_target(uuid)
  to authenticated;

comment on function public.resolve_my_notification_target(uuid)
is 'Resolves an owned notification to the caller current authoritative Plan or Signal state, including the current Signal coordination stage.';
