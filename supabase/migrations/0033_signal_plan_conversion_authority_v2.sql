...
-- SIGNAL
-- Migration 0033
-- Atomic Signal -> Plan conversion on authoritative venue/time
-- ============================================================
-- Replaces the legacy group_decisions conversion dependency with
-- the venue/time authorities introduced in 0030 and 0031.
-- ============================================================
...
create unique index if not exists venues_external_source_place_key
  on public.venues(external_source, external_place_id);

alter table public.venues
  drop constraint if exists venues_external_identity_check;

alter table public.venues
  add constraint venues_external_identity_check
  check (
    (external_source is null and external_place_id is null)
    or
    (nullif(btrim(external_source), '') is not null
      and nullif(btrim(external_place_id), '') is not null)
  );
