...
as $$
  ...
  active_intents as (
    ...
    join public.activities intent_activity on intent_activity.id = si.activity_id
    where si.state in ('active', 'assigned')
      and si.expires_at > now()
      and up.completion_state = 'complete'
      and (
        intent_activity.slug <> 'chill'
        or si.user_id = au.user_id
        or public.chill_users_are_reciprocally_compatible(au.user_id, si.user_id)
      )
      and not exists (
  ...
  ranked_avatar_candidates as (
    ...
    from active_intents ai
    join public.activities preview_activity on preview_activity.id = ai.activity_id
    where ai.avatar_path is not null
      and nullif(btrim(ai.avatar_path), '') is not null
      -- Chill candidate identity is private until the reciprocal pair locks.
      -- Avatar object paths include owner-folder identity, so they must not
      -- leave PostgreSQL through discovery even if the frontend hides them.
      and preview_activity.slug <> 'chill'
  ),
...
is 'Returns block-safe live discovery social proof. Chill counts only reciprocal candidates and returns no pre-lock avatar object paths.';
commit;
