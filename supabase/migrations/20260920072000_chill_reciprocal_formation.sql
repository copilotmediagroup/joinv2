...
create or replace function public.chill_users_are_reciprocally_compatible(
  p_user_a uuid,
  p_user_b uuid
)
returns boolean
...
as $$
  with people as (
    select up.user_id, up.gender,
      extract(year from age(current_date,up.birth_date))::integer as age,
      cdp.seeking_gender, cdp.min_age, cdp.max_age, cdp.is_enabled
    from public.user_profiles up
    join public.chill_dating_preferences cdp on cdp.user_id=up.user_id
    where up.user_id in (p_user_a,p_user_b)
      and up.completion_state='complete' and up.birth_date is not null
  ), a as (select * from people where user_id=p_user_a),
     b as (select * from people where user_id=p_user_b)
  select coalesce((select a.is_enabled and b.is_enabled and a.age>=18 and b.age>=18
    and a.seeking_gender=b.gender and b.seeking_gender=a.gender
    and b.age between a.min_age and a.max_age
    and a.age between b.min_age and b.max_age
    and not public.users_have_block_relation(p_user_a,p_user_b)
    from a cross join b),false);
$$;

revoke all on function public.chill_users_are_reciprocally_compatible(uuid,uuid) from public,anon,authenticated;
grant execute on function public.chill_users_are_reciprocally_compatible(uuid,uuid) to service_role;

-- Assigned-user retry fast path: recover an already-authoritative live assignment
-- before entering named-window formation serialization. First-time formation remains
-- unchanged and still uses both proven concurrency locks.
...
declare
  ...
  v_delegate_max_age integer := p_max_age;
  ...
  v_existing_group_state public.signal_group_state;
  v_existing_member_count integer;
  v_existing_activation_threshold integer;
...
begin
  ...
  if v_activity.slug = 'chill' then
    if not exists (
      select 1 from public.chill_dating_preferences cdp
      where cdp.user_id=p_user_id and cdp.is_enabled=true
    ) then
      raise exception 'chill_dating_preferences_required';
    end if;

    -- Dating age ranges are reciprocal private predicates, not group identity.
    v_effective_min_age := null;
    v_delegate_max_age := null;
  end if;
  ...
  -- ==========================================================
  -- ASSIGNED-USER RETRY FAST PATH
  --
  -- A caller that already owns a live assignment does not need to
  -- queue behind unrelated first-time callers for the same named
  -- window. Lock the authoritative assignment rows in SHARE mode,
  -- which is compatible with concurrent readers but blocks writers
  -- until this short transaction returns. New group admission still
  -- takes the named-window advisory lock below.
  -- ==========================================================
  ...
  into
    ...
    v_existing_group_state,
    v_existing_activation_threshold
  ...
  where si.user_id = p_user_id
    ...
    and si.max_age is not distinct from v_delegate_max_age
  ...
  limit 1
  for share of si, sgm, sg;

  if v_existing_group_id is not null then
    select count(*)::integer
    into v_existing_member_count
    from public.signal_group_memberships active_membership
    where active_membership.signal_group_id = v_existing_group_id
      and active_membership.state in ('matched', 'confirmed');
  ...
  perform pg_advisory_xact_lock(
    hashtextextended(
      concat_ws(
        ...
        coalesce(v_delegate_max_age::text, '<null>'),
        v_time_window_code,
        v_time_window_key
      ),
      0
  ...
  if v_existing_group_id is not null then
    ...
    where sg.city_id = v_city.id
      ...
      and sg.max_age is not distinct from v_delegate_max_age
      and sg.time_window_code = v_time_window_code
      and sg.time_window_key = v_time_window_key
