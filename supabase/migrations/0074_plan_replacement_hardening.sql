...
-- Replacement windows are server-owned. Browser access is RPC-only.
alter table public.plan_replacement_windows enable row level security;
revoke all on table public.plan_replacement_windows from anon, authenticated;
grant all on table public.plan_replacement_windows to service_role;

-- Resume authority must follow active Plan membership, not stale Signal history.
  ...
  with journey_candidates as (
    ...
    select
      ...
      case
        ...
        when exists(select 1 from public.signal_venue_rounds svr where svr.signal_group_id=sg.id and svr.state in ('open','runoff')) then 'places'
        when exists(select 1 from public.signal_venue_rounds svr where svr.signal_group_id=sg.id and svr.state='won' and svr.winner_option_id is not null) then 'time'
        else 'arrival'
      end,
      (
        select svo.payload
