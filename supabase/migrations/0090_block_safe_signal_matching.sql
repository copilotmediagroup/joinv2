...
begin
  if auth.uid() is null or p_user_id is distinct from auth.uid() then
    raise exception 'signal_block_compatibility_access_denied' using errcode='42501';
  end if;
...
revoke all on function public.is_my_signal_group_block_compatible(uuid,uuid) from public,anon;
grant execute on function public.is_my_signal_group_block_compatible(uuid,uuid) to authenticated;

create or replace function public.enforce_signal_membership_block_guard()
returns trigger
language plpgsql
security definer
set search_path=public,pg_temp
as $function$
begin
  if new.state in ('matched','confirmed') and exists (
    select 1
    from public.signal_group_memberships existing
    join public.user_blocks ub
      on (ub.blocker_user_id=new.user_id and ub.blocked_user_id=existing.user_id)
      or (ub.blocked_user_id=new.user_id and ub.blocker_user_id=existing.user_id)
    where existing.signal_group_id=new.signal_group_id
      and existing.id is distinct from new.id
      and existing.state in ('matched','confirmed')
  ) then
    raise exception 'signal_membership_blocked_pair' using errcode='42501';
  end if;
  return new;
end;
$function$;

alter function public.enforce_signal_membership_block_guard() owner to postgres;
revoke all on function public.enforce_signal_membership_block_guard() from public,anon,authenticated;

drop trigger if exists signal_membership_block_guard on public.signal_group_memberships;
create trigger signal_membership_block_guard
before insert or update of signal_group_id,user_id,state on public.signal_group_memberships
for each row execute function public.enforce_signal_membership_block_guard();
