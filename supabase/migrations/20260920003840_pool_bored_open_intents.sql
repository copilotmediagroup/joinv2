...
begin
  ...
  if v_city is null then raise exception 'complete_profile_with_active_city_required' using errcode='P0001'; end if;  update public.bored_open_intents
  ...
  with recent_activity as (
    ...
    left join recent_activity ra on ra.activity_id=a.id    where a.is_active=true
...
is 'Automated NOW opportunity engine: pools serialized open boredom intents by city and activity, combines them with live Signal demand, local-time fit, age, blocks, recent history and explicit passes.';
