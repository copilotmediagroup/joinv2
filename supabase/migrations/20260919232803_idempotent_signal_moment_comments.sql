...
alter table public.signal_moment_comments
  add column if not exists client_comment_id uuid;

create unique index if not exists signal_moment_comments_author_client_key
  on public.signal_moment_comments(author_user_id,client_comment_id)
  where client_comment_id is not null;

drop function if exists public.add_signal_moment_comment(uuid,text,uuid);

create function public.add_signal_moment_comment(
  p_moment_id uuid,
  p_body text,
