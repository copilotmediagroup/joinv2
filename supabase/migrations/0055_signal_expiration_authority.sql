...
-- Backfill stale pre-authority groups without sending historical notifications.
alter table public.signal_groups disable trigger signal_group_state_notification;
select * from public.reconcile_expired_signal_groups(5000);
alter table public.signal_groups enable trigger signal_group_state_notification;

select cron.unschedule(jobid)
from cron.job
where jobname='signal-expiration-reconcile';

select cron.schedule(
  'signal-expiration-reconcile',
  '* * * * *',
  'select public.reconcile_expired_signal_groups(500);'
);
