...
-- If a notification references a Signal the caller has since left,
-- resolve to the caller's current live Signal instead of returning
-- a dead target that appears to do nothing in the browser.
...
begin
  ...
  if v_signal_intent_id is null then
    ...
    where sgm.user_id=v_user_id
      and sgm.state in ('matched','confirmed')
      and sg.state in ('forming','confirming','locked','coordinating','active_outing')
    order by sgm.created_at desc
    limit 1;
  end if;
...
is 'Resolves owned notifications to the current authoritative Plan or live Signal, falling back from stale Signal alerts to the caller current live Signal.';
