...
-- SIGNAL
-- Migration 0032
-- Venue recovery after no-eligible-time outcomes
-- ============================================================
-- Failed venue choices are durably excluded for this Signal.
-- The current time round and venue decision rounds are reset so
-- signal-places can create a fresh authoritative round without
-- returning the failed venue again.
-- ============================================================

create table if not exists public.signal_venue_exclusions (
  ...
  primary key (signal_group_id, place_id),
  constraint signal_venue_exclusions_place_check
    check (nullif(btrim(place_id), '') is not null),
  constraint signal_venue_exclusions_reason_check
    check (reason in ('closed', 'no_eligible_time', 'unavailable'))
);

alter table public.signal_venue_exclusions enable row level security;

create policy signal_venue_exclusions_select_member
on public.signal_venue_exclusions
for select to authenticated
using (public.is_signal_group_member(signal_group_id));

revoke all on table public.signal_venue_exclusions from anon;
revoke insert, update, delete on table public.signal_venue_exclusions from authenticated;
grant select on table public.signal_venue_exclusions to authenticated;
...
begin
  ...
  if p_signal_group_id is null or nullif(btrim(p_failed_place_id), '') is null then
  ...
  if v_winning_place_id <> btrim(p_failed_place_id) then
    raise exception 'signal_venue_recovery_place_mismatch' using errcode = 'P0001';
  end if;

  insert into public.signal_venue_exclusions (
    signal_group_id, place_id, reason, excluded_at
  ) values (
    v_group.id, v_winning_place_id, p_reason, clock_timestamp()
  )
  on conflict (signal_group_id, place_id)
  do update set reason = excluded.reason, excluded_at = excluded.excluded_at;

  delete from public.signal_time_rounds
