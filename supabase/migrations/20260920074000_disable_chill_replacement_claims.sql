...
begin
  ...
  -- Chill is an intentional reciprocal two-person date. A vacated seat cannot
  -- be filled by generic post-lock replacement admission.
  if lower(btrim(p_activity_slug))='chill' then
    return query select null::uuid,false;
    return;
  end if;

  -- Serialize first-time replacement claims per user; no membership row exists yet to lock.
  perform pg_advisory_xact_lock(hashtextextended(
    concat_ws('|','plan_replacement_user_v1',v_user_id::text),0
  ));
  ...
  -- Choose without locking the window first. Reconciliation owns Plan -> window.
  ...
  order by prw.opened_at,prw.id limit 1
  for update of p skip locked;

  if v_plan_id is null then return query select null::uuid,false; return; end if;

  -- Revalidate authority after the Plan lock; another claimant may have filled it.
  perform 1
  from public.plan_replacement_windows prw
  join public.plans p on p.id=prw.plan_id
  where prw.plan_id=v_plan_id
    and prw.state='open'
    and prw.deadline_at>clock_timestamp()
    and p.state='recovery_required'::public.plan_state
    and p.scheduled_starts_at>clock_timestamp()+interval '30 minutes'
  for update of prw;
...
$function$;
alter function public.claim_matching_plan_replacement(text,text,text,public.crowd_mode,integer,integer) owner to postgres;
...
is 'Retry-safe replacement claim serialized per user; candidate Plans use SKIP LOCKED before replacement-window locking and post-lock authority revalidation.';
