...
begin
  ...
  with recent_activity as (
    ...
    where si.city_id=v_city and si.state in ('active','assigned') and si.expires_at>clock_timestamp()
      and si.user_id<>v_user and not public.users_have_block_relation(v_user,si.user_id)
      and (
        not exists(select 1 from public.activities ax where ax.id=si.activity_id and ax.slug='chill')
        or public.chill_users_are_reciprocally_compatible(v_user,si.user_id)
      )
    ...
    where boi.city_id=v_city and boi.state='open'
      ...
      and (
        not exists(select 1 from public.activities ax where ax.id=boi.chosen_activity_id and ax.slug='chill')
        or public.chill_users_are_reciprocally_compatible(v_user,boi.user_id)
      )
    ...
    select a.id,a.slug,a.name,
      ...
      (coalesce(ls.n,0)*100 + coalesce(bp.n,0)*110
       ...
       + case
          ...
          when (v_hour>=22 or v_hour<4) and a.slug in ('nightlife','drinks','music','chill') then 60
    ...
    where a.is_active=true
      ...
      and (
        a.slug<>'chill'
        or exists (
          select 1 from public.signal_intents partner
          where partner.city_id=v_city and partner.activity_id=a.id
            and partner.state in ('active','assigned') and partner.expires_at>clock_timestamp()
            and partner.user_id<>v_user
            and public.chill_users_are_reciprocally_compatible(v_user,partner.user_id)
        )
        or exists (
          select 1 from public.bored_open_intents partner
          where partner.city_id=v_city and partner.state='open'
            and partner.expires_at>clock_timestamp() and partner.user_id<>v_user
            and public.chill_users_are_reciprocally_compatible(v_user,partner.user_id)
        )
      )
      and (
        not (v_hour>=22 or v_hour<4)
        or a.slug in ('nightlife','drinks','music','chill')
      )
  )
  select r.id,r.slug,r.name,r.active_n,
    case when r.active_n>0 then 'people_active'
         ...
         when (v_hour>=22 or v_hour<4) and r.slug in ('nightlife','drinks','music','chill') then 'late_night_fit'
...
is 'Automated NOW opportunity engine. Formation-eligible activities are filtered and ranked by city-local time; late night spans 22:00 through 03:59 and excludes daytime-only activity classes.';
