...
begin  if v_user_id is null then
  ...
  perform pg_advisory_xact_lock(
    hashtextextended(concat_ws('|','signal_moment_capture_v1',p_plan_id::text,v_user_id::text),0)
  );
  ...
  -- Match lifecycle lock order (Plan -> membership) before trusting capture
  -- eligibility. This prevents completion/leave from racing a stale authority
  -- check and leaving a draft Moment after the Plan has already completed.
  perform 1
  from public.plans p
...
comment on function public.capture_my_signal_moment(uuid,text,jsonb) is 'Serializes first Moment capture per Plan/member before absent-row selection; repeat media paths remain idempotent and the six-item cap remains authoritative.';
